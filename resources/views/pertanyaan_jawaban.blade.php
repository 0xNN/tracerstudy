...
@section('konten')
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">Pertanyaan Jawaban</h5>
        ...
        <div class="modal-body">
          <div class="form-group">
            ...
            <select name="pertanyaan_id" id="pertanyaan_id" class="form-control form-control-sm">
              <option>-- Pertanyaan --</option>
              @foreach ($pertanyaans as $item)
                <option value="{{ $item->id }}">{{ $item->id }} - {{ $item->kode_pertanyaan }} - {{ $item->deskripsi_pertanyaan }}</option>
              @endforeach
            </select>
          </div>
          <div class="form-group">
            <label for="sub_pertanyaan_id">Sub Pertanyaan</label>
            <select name="sub_pertanyaan_id" id="sub_pertanyaan_id" class="form-control form-control-sm">
              <option>-- Sub Pertanyaan --</option>
              @foreach ($sub_pertanyaans as $item)
                <option value="{{ $item->id }}">{{ $item->deskripsi_sub_pertanyaan }}</option>
              @endforeach
            </select>
          </div>
          <div class="form-group">
            <input type="hidden" name="id" id="id">
            <label for="jawaban_id">Jawaban</label>
            <select name="jawaban_id" id="jawaban_id" class="form-control form-control-sm">
              <option>-- Jawaban --</option>
              @foreach ($jawabans as $item)
                <option value="{{ $item->id }}">{{ $item->deskripsi_jawaban }}</option>
              @endforeach
            </select>
          </div>
        </div>
  ...
  <div class="d-sm-flex align-items-center justify-content-start mb-4">
    <h1 class="h3 mb-0 text-gray-800"><i class="fas fa-question-circle"></i>Data</h1>
  </div>
  <div class="card shadow mb-4">
    <div class="card-header py-3 ">
        <h6 class="m-0 font-weight-bold text-primary d-sm-inline-block">Pertanyaan Jawaban</h6>
    </div>
    <div class="card-body">
  ...
  <script>
    ...
    $(document).ready( function () {
      $('#utama').DataTable({
        ...
        serverSide: true,
        ajax :{
          url:"{{ route('pertanyaan_jawaban.index') }}",
          type:'GET',
        },
        columns: [
          {data: 'id', name: 'id'},
          {data: 'pertanyaan_id', name: 'pertanyaan_id'},
          {data: 'sub_pertanyaan_id', name: 'sub_pertanyaan_id'},
          {data: 'jawaban_id', name: 'jawaban_id'},
            ...
            $("#form-tambah-edit").validate({
                submitHandler: function (form) {
                    ...
                    $.ajax({
                        ...
                        url: "{{ route('pertanyaan_jawaban.store') }}", //url simpan data
